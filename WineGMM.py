...
"""
            Algorithm for Wine Quality Data Sourced at Kaggle
            ...
            Data URL:https://www.kaggle.com/uciml/red-wine-quality-cortez-et-al-2009/version/2

            Github:
            

            Note: Expects data to be extracated in  ~/Datasets/winequality/winequality-red.csv' relative to the path of this file. 
"""
...
output_dir = os.path.join(script_dir, 'Results/Wine')
...
dfmain = pd.read_csv('Datasets/winequality/winequality-red.csv')

#Sorting and Separating Data
x1 = np.sort(dfmain['sulphates'].values)

## Plotting
ax1 = sns.distplot(x1, bins=50, kde=False)
ax1.set(xlabel='Sulphates in Red Wine', ylabel='Occurences')
plt.title('Histogram of Sulphates in Red Wine')
...
for k in range(K_Search):
    ...
    ax.legend()
    ax.set(xlabel='Sulphates in Red Wine', ylabel='Relative Probabilitty')
    plt.title('Histogram of Sulphates in Red Wine With Random Distribution Guess')
...
############### HOW MANY ITERATIONS? #################
    ...
    ax.legend()
    ax.set(xlabel='Sulphates in Red Wine', ylabel='Relative Probabilitty')
    plt.title('Histogram of Sulphates in Red Wine After EM Algo')
